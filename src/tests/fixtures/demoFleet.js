/**
 * The demonstration fleet, kept as a TEST FIXTURE only.
 *
 * These six farmers and eight systems used to be the app's starting state:
 * every browser that opened the platform was seeded with them. None of it was
 * real — no unit sat behind BRD001..BRD008 and the farmers were invented — so
 * it has been removed from the app. The app's real starting state is
 * `buildSeed()` in src/lib/seed.js. Nothing under src/lib imports this file, so
 * none of it reaches the shipped bundle, and src/tests/demo-removed.test.jsx
 * asserts exactly that.
 *
 * What it is for: the suites around it pin behaviour that needs a populated
 * fleet — a plan shorter than its batch, a locked unit, a churn candidate — and
 * those are real product states even though the records are not. A suite that
 * needs them imports this file deliberately, by this path.
 *
 * All dates are generated relative to "now" so the fixture always looks live.
 ...
 * money. The fixture therefore also carries the two cases a per-batch
 ...
 */
import { addDays, fmtDate } from '../../lib/time.js';
import { TERMS, bandForChicks, priceFor } from '../../lib/subscriptions.js';
...
/**
 * The app's real initial state with the demonstration fleet laid over it — what
 * a suite that needs a populated platform builds from. The console account, the
 * plan catalogue and the price sheet come from `buildSeed()` itself and are
 * deliberately not repeated here, so the fixture cannot drift from the real one
 * (or arrive carrying a version marker that is not the app's).
 */
