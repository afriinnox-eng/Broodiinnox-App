/**
 * MTN MoMo payments in the farmer's app, collected by the Ekorana gateway:
 * request -> provider -> confirm -> unlock.
 *
 ...
 *               when the gateway's own answer says `successful` AND that answer
 *               is the provider's (`provider_confirmed: true`) — a row that
 *               merely claims success is still pending.
 ...
 *               neither charges nor prompts again; the gateway's refusal is
 *               recorded with what it said; a provider-confirmed payment
 *               activates the subscription the farmer paid for.
 ...
 *               be checked with the gateway from the page, and a server that
 *               cannot take payments says so instead of collecting a number.
 */
...
const apiRow = (over = {}) => ({
  ...
  provider_ref: 'pay_api_1',
  financial_transaction_id: 'EK-TX-1',
  reason: null,
...
let calls = [];
/** What the fake broodiinnox-api answers this test. `provider` is its health report. */
let server = {};
...
beforeEach(async () => {
  ...
  server = { provider: { enabled: true, missing: [], invalid: [] }, create: apiRow(), createStatus: 201 };
  vi.unstubAllEnvs();
  ...
  vi.stubGlobal('fetch', vi.fn(async (url, opts = {}) => {
    ...
    if (method === 'GET' && u.endsWith('/api/health')) {
      // The API reports its gateway configuration under `ekopay` — names only.
      return jsonResponse({ ok: true, ekopay: server.provider });
    }
...
describe('INVARIANT: the app never confirms a payment the provider has not', () => {
  ...
  it('carries only the provider-derived fields, and never a confirmation', () => {
    ...
    // The gateway has only received the request: nothing here reads as paid.
    const pending = providerFieldsFromRow(apiRow());
    expect(pending).toMatchObject({
      apiId: 'pay_api_1', providerRef: 'pay_api_1', providerConfirmed: false, providerStatus: 'pending',
    });
    expect(pending.financialTxId).toBe('EK-TX-1');
    expect(pending.amount).toBe(PRICE);

    // A row that CLAIMS success without the provider's own flag carries no
    // confirmation: that flag comes from the gateway's answer and from nothing
    // else — not even a `financial_transaction_id` on the row.
    ...
    // Fields the app has no business copying are ignored, and the gateway's own
    ...
    const real = apiRow({ status: 'successful', provider_confirmed: true, financial_transaction_id: 'EK-TX-77' });
    expect(isProviderConfirmed(real)).toBe(true);
    expect(providerFieldsFromRow(real).financialTxId).toBe('EK-TX-77');
  ...
  it('says in plain words why a payment did not go through', () => {
    const { paymentFailureNote, paymentPendingNote, providerDisabledNote } = pay;
    expect(paymentFailureNote({ failureReason: 'INVALID_PHONE' })).toMatch(/not an MTN MoMo number/);
    expect(paymentFailureNote({ failureReason: 'AMOUNT_MISMATCH' })).toMatch(/different amount/);
    expect(paymentFailureNote({ failureReason: 'REFERENCE_EXISTS' })).toMatch(/already requested/);
    expect(paymentFailureNote({ failureReason: 'INVALID_API_KEY' })).toMatch(/API key/);
    expect(paymentFailureNote({ failureReason: 'Payments are not configured on the server yet.' }))
      .toBe('Payments are not configured on the server yet.');
    ...
    expect(providerDisabledNote({})).toMatch(/not configured on the server/);
  });
});
...
describe('BEHAVIOURAL: a real payment goes to the gateway and back', () => {
  ...
  it('asks the API for the sheet price, then unlocks the system only on the gateway\'s confirmation', async () => {
    ...
    // The gateway has only received it: the system is NOT unlocked yet.
    const pending = probe.state.payments.find((p) => p.deviceId === DEVICE && p.provider === true);
    expect(pending).toMatchObject({ status: 'pending', providerRef: 'pay_api_1', providerConfirmed: false });
    expect(device(DEVICE).subscription.status).toBe('expired');

    // The provider answers: successful, on its own authority.
    server.status = apiRow({
      status: 'successful', provider_confirmed: true, financial_transaction_id: 'EK-TX-77',
      confirmed_at: '2026-09-11T08:01:00.000Z',
    });
    await settle(5100);

    const paid = probe.state.payments.find((p) => p.id === pending.id);
    expect(paid).toMatchObject({ status: 'successful', providerConfirmed: true, financialTxId: 'EK-TX-77', failureReason: null });
  ...
  it('does not unlock on a success that the provider has not confirmed', async () => {
    ...
    const p = probe.state.payments.find((x) => x.deviceId === DEVICE && x.provider === true);
  ...
  it("records the gateway's refusal, in its own words, with nothing unlocked", async () => {
    seedWith(FARMER);
    render(<StoreProvider><Probe /></StoreProvider>);
    const before = device(DEVICE).subscription;

    server.createStatus = 502;
    server.create = apiRow({ status: 'failed', provider_confirmed: false, reason: 'INVALID_PHONE' });
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (url, opts = {}) => {
      ...
      if (method === 'POST' && u.endsWith('/api/payments')) {
        return jsonResponse({ error: 'That number is not one the payment gateway can charge — check it and try again.', payment: server.create }, 502);
      }
      if (method === 'GET' && u.endsWith('/api/health')) return jsonResponse({ ok: true, ekopay: server.provider });
    ...
    expect(p.providerConfirmed).toBe(false);
    expect(p.providerRef).toBe('pay_api_1'); // the reference the gateway knows, kept
    expect(p.providerStatus).toBe('failed');
    expect(pay.paymentFailureNote(p)).toBe('That number is not one the payment gateway can charge — check it and try again.');
...
describe('FUNCTIONAL: the payment screens', () => {
  it('refuses an unreachable number in the payment modal, and asks the gateway for a good one', async () => {
    seedWith(FARMER);
  ...
  it('lets the farmer check a pending payment with the gateway from the Payments page', async () => {
    seedWith(FARMER, (seed) => ({
      ...seed,
      payments: [{
        ...
        providerConfirmed: false, providerRef: 'pay_api_1', period: '30-Day Plan', createdAt: '2026-09-11T08:00:00.000Z',
        provider: true, currency: 'RWF', apiId: 'pay_api_1', financialTxId: null, failureReason: null,
    ...
    // The gateway confirms on the status check the farmer asked for.
    server.refresh = apiRow({
      status: 'successful', provider_confirmed: true, financial_transaction_id: 'EK-TX-88',
    });
    ...
    expect(mine).toMatchObject({ status: 'successful', providerConfirmed: true, financialTxId: 'EK-TX-88' });
  ...
  it('says so, and offers no way to pay, when the server cannot collect payments', async () => {
    server.provider = { enabled: false, missing: ['EKOPAY_API_KEY', 'EKOPAY_TRANSFER_PHONE'], invalid: [] };
    seedWith(FARMER);
    const { default: FarmerPayments } = await import('../pages/farmer/Payments.jsx');
    render(<StoreProvider><FarmerPayments /></StoreProvider>);
    await settle(100);

    expect(screen.getByText(/not configured on the server yet \(missing: EKOPAY_API_KEY, EKOPAY_TRANSFER_PHONE\)/))
      .toBeInTheDocument();
