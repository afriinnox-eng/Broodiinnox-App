 ...
 * POST /api/payments                          — request a payment via MTN MoMo.
 *
 * POST body: { device_id, amount, phone, farmer_id?, plan_id?, band_id?, currency? }
 *
 * The amount is the price the app published for that farm size and plan; MoMo
 * is asked to collect exactly it, and a payment is confirmed only when MoMo
 * says the same amount, in the same currency, arrived.
 *
 * 201 the prompt was sent · 200 a live prompt was reused (no second charge)
 * 400 the request is not payable · 404 unknown device · 502 MoMo refused it
 * 503 MTN MoMo is not configured on this server (the body names the missing env vars)
 */
